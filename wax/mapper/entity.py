...
class EntityMapper(Mapper):
    @select_one('''select id from tbl_entity where project_id=:project_id and iid=:iid''')
    ...
    @insert('''insert tbl_entity(id, iid, project_id, superset_iid, name, content)
    values (:id, :iid, :project_id, :superset_iid, :name, :content)
    ''')
    async def insert_entity(self, *, id: int, iid: int, project_id: int, superset_iid: int, name: str, content: str):
        pass
